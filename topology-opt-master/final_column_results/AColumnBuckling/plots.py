...
def build_subplot():
    ...
    A = dict()
    A[0] = np.loadtxt('new_results_txt\AColumnBuckling\A1_final[buckling=0].dat')
    A[1] = np.loadtxt('new_results_txt\AColumnBuckling\A2_final_Column 60x30[p=3, buckling=0.080, vol=0.3]_densities.dat]_densities.dat')
    A[2] = np.loadtxt('new_results_txt\AColumnBuckling\Changed_A3 60x30[p=3, buckling=0.106, vol=0.3]_densities.dat')
    A[3] = np.loadtxt('new_results_txt\AColumnBuckling\A4_final_Column 60x30[p=3, buckling=0.133, vol=0.3]_densities.dat]_densities.dat')
    ...
    fig, ((axs[0], axs[1]), (axs[2], axs[3])) = plt.subplots(2, 2, figsize=(4,6))
